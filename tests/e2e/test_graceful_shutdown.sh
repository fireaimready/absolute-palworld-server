...
test_graceful_shutdown() {
    ...
    local logs_before
    logs_before=$(docker logs palworld-server 2>&1 | wc -l)

    # Send graceful shutdown signal
    log_info "Sending graceful shutdown signal (SIGINT)"
    docker kill --signal=INT palworld-server || true
    ...
    while [[ ${elapsed} -lt ${timeout} ]]; do
        local logs
        logs=$(docker logs palworld-server 2>&1)
        ...
        if echo "${logs}" | grep -qi "stopped.*palworld\|palworld.*stopped\|exited\|terminated\|SIGINT"; then
            process_stopped=true
            log_info "Process stop/termination detected in logs"
        fi

        # Check for save (may happen during shutdown)
        if echo "${logs}" | grep -qi "Saving\|World save"; then
            log_success "Save detected during shutdown"
        fi
    ...
    container_running=$(docker inspect -f '{{.State.Running}}' palworld-server 2>/dev/null || echo "false")

    # Get final logs for verification
    local final_logs
    final_logs=$(docker logs palworld-server 2>&1)
    ...
    if [[ "${container_running}" != "true" ]]; then
        ...
        # Check exit code
        local exit_code
        exit_code=$(docker inspect -f '{{.State.ExitCode}}' palworld-server 2>/dev/null || echo "unknown")
    ...
    if [[ "${container_running}" == "true" ]]; then
        ...
        docker stop --time=30 palworld-server 2>/dev/null || true
    fi
