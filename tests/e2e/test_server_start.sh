...
test_server_start() {
    ...
    if wait_for_log "palworld-server" "Starting Palworld server update" 120; then
    ...
    while [[ ${attempts} -lt ${max_attempts} ]]; do
        ...
        if [[ $(docker inspect -f '{{.State.Running}}' palworld-server 2>/dev/null) != "true" ]]; then
            ...
            docker logs palworld-server 2>&1 || true
            log_error "=== End Container Logs ==="
            return 1
        fi

        # Use bash -c to avoid Git Bash path conversion on Windows
        if MSYS_NO_PATHCONV=1 docker exec palworld-server test -f /opt/palworld/server/PalServer.sh 2>/dev/null; then
