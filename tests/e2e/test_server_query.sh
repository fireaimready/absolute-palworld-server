...
test_server_query() {
    ...
    # Get the server port
    local server_port="${SERVER_PORT:-8211}"
    ...
    while [[ ${attempts} -lt ${max_attempts} ]]; do
        local udp_sockets
        udp_sockets=$(MSYS_NO_PATHCONV=1 docker exec palworld-server cat /proc/net/udp 2>/dev/null || echo "")

        local port_8211_ok=false
        local port_27015_ok=false

        # Check for query port (27015) - this is the Steam query port
        if echo "${udp_sockets}" | grep -qi ":${query_port_hex}"; then
            port_27015_ok=true
        fi

        # For the game port (8211)
        if echo "${udp_sockets}" | grep -qi ":${port_hex}"; then
            port_8211_ok=true
        fi

        # Success if query port is bound (main indicator that server is ready)
        if [[ "${port_27015_ok}" == "true" ]] || [[ "${port_8211_ok}" == "true" ]]; then
            if [[ "${port_27015_ok}" == "true" ]]; then
                log_success "Server is listening on query port ${query_port}"
            fi
            if [[ "${port_8211_ok}" == "true" ]]; then
